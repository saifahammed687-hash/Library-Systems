...
router.post("/register", async (req, res) => {
  try {
    ...
    const allowed = ["student", "teacher", "librarian", "admin"];
...
router.post("/login", async (req,res) => {
  try {
    ...
    const u = rows[0];
    const allowed = ["student", "teacher", "librarian", "admin"];
    const chosenRole = allowed.includes(role) ? role : u.role;
    req.session.user = { id:u.id, name:u.name, email:u.email, role:chosenRole };
    res.json({ user:req.session.user });
